            ...
            for c in self.cargos:
                for p in self.planes:
                    for a in self.airports:
                        ...
                        predcond_pos = [expr('At({}, {})'.format(c,a)),expr('At({}, {})'.format(p,a))]
                        predcond_neg = []
                        effect_add = [expr('In({}, {})'.format(c,p))]
                        effect_remove = [expr('At({}, {})'.format(c,a))]
                        load_action = Action(expr('Load ({}, {}, {})'.format(c,p,a)),[predcond_pos,predcond_neg],[effect_add,effect_remove])
            ...
            for airport in self.airports:
                for plane in self.planes:
                    for cargo in self.cargos:
                        pos = [expr("At({}, {})".format(plane, airport)),
            ...
            for c in self.cargos:
                for p in self.planes:
                    for a in self.airports:
                        ...
                        #   EFFECT:At(c,a) & ~In(c,p)
                        predcond_pos = [expr('In({}, {})'.format(c,p)),expr('At({}, {})'.format(p,a))]
                        predcond_neg = []
                        effect_add = [expr('At({}, {})'.format(c,a))]
                        effect_remove = [expr('In({}, {})'.format(c,p))] 
                        unload_action = Action(expr('Unload ({}, {}, {})'.format(c,p,a)),[predcond_pos,predcond_neg],[effect_add,effect_remove])
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    # where the cargo and planes are
    pos = [expr('At(C1, SFO)'),
           expr('At(C2, JFK)'),
           expr('At(C3, ATL)'),
           expr('At(C4, ORD)'),
    ...
    # where the cargo and planes are not
    neg = [expr('At(C1, JFK)'),
           expr('At(C1, ATL)'),
           expr('At(C1, ORD)'),
           expr('In(C1, P1)'),
